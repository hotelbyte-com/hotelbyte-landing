          ...
          >
            ...
            >
              <div className={`w-12 h-12 rounded-sm flex items-center justify-center mb-6 group-hover:scale-110 transition-transform duration-500 ${
                product.color === 'brass' ? 'bg-brass/10' :
                product.color === 'seal' ? 'bg-seal/10' :
                'bg-paper-raised'
              }`}>
                <product.icon className={`w-6 h-6 ${
                  product.color === 'brass' ? 'text-brass' :
                  product.color === 'seal' ? 'text-seal' :
                  'text-ink'
                }`} />
              </div>
              <div className="text-xs font-medium text-ink/40 mb-2">{isEn ? product.shortDescEn : product.shortDesc}</div>
              <h3 className="text-2xl font-display mb-4">{isEn ? product.titleEn : product.title}</h3>
              <p className="text-ink/60 leading-relaxed mb-6">{isEn ? product.descEn : product.desc}</p>
